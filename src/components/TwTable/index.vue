<template>
    <div id="tw_table">
                        ...
                        <template #dropdown>
                            <el-dropdown-menu>
                                <template v-for="(item, index) in headBtns" :key="index">
                                    <el-dropdown-item
                                        v-if="index > 1 && (item.formatter ? item.formatter(item, index) : true)"
        ...
        <el-table ref="table" :id="id" :data="dataList" :height="getH" :highlight-current-row="highlightCurrentRow" :size="size" :stripe="stripe" :border="border"
            v-loading="loading" element-loading-text="加载中..." @row-click="rowCheck" @select="checkSelect" :row-class-name="rowClassName"
            @selection-change="selectionChange" @current-change="tableCurrentChange" center class="w">
            <!-- 多选 -->
            <el-table-column current-row-key="selection" align="center" :fixed="checkFixed" header-align="center"
            ...
            <template v-for="(item, index) in columns" :key="index">
                <template v-if="!item.hidden">
                    <template v-if="item.formatter">
                        <el-table-column resizable show-overflow-tooltip :type="item.type" :label="item.label"
                                         :sortable="item.sortable" :width="item.width" :fixed="item.fixed" :class-name="item.className"
                                         :render-header="item.renderHeader" :formatter="item.formatter">
                        </el-table-column>
                    </template>
                            ...
                            <template v-if="scope.row?.[item.prop] || scope.row?.[item.prop] == 0">
                                <template v-if="!!item.color">
                                    <span :style="`color: ${item.color}`">
                                        {{!!item.filter ? $store.getters.oEnumerate?.[item.filter]?.[scope.row[item.prop]]
                                        : scope.row?.[item.prop] }}
                                    </span>
                                </template>
                                <template v-else>
            ...
            <template v-if="tableBtns.prop">
                <el-table-column resizable show-overflow-tooltip :prop="tableBtns.prop" :type="tableBtns.type"
                    :label="tableBtns.label" :width="tableBtns.width" :fixed="tableBtns.fixed" header-align="center">
                    <!-- 自定义 header (使用组件渲染 tsx 语法) -->
                    ...
                    <template #default="{ row, column, $index }">
                        <p class="tw_cl" style="flex-wrap: nowrap;">
                            <template v-for="(item, i) in tableBtns.btns">
                                <template v-if="item.formatter">
                                    <el-tooltip :hide-after="0" v-if="item.formatter(row, column, $index)" :key="i" effect="dark"
                                        placement="top-start">
                                        <template #content> {{ item.name }}</template>
                                        <el-icon @click="item.click(row, column, $index)" size="20px"
                                            class="pointer mr10">
                                            <svg-icon :icon-class="item.icon" :color="item.color"></svg-icon>
                                        </el-icon>
                                    </el-tooltip>
                                </template>
                                <el-tooltip v-else :hide-after="0" :key="i" effect="dark" placement="top-start">
                                    <template #content> {{ item.name }}</template>
                                    <el-icon @click="item.click(row, column, $index)" size="20px" class="pointer mr10">
                                        <svg-icon :icon-class="item.icon" :color="item.color"></svg-icon>
                                    </el-icon>
                                </el-tooltip>
                            </template>
        ...
        <!--  分页  -->
        <tw-pagination v-if="showPagination" v-model:currentPage="tableParams.pageNumber"
            v-model:pageSize="tableParams.pageSize" :total="total" :small="small" :pageSizesArr="pageSizesArr"
...
// @ts-ignore
const { url, id, method, columns, lists, tableParams, showPagination, botHeight, indexFixed, isCheck, checkFixed, showIndex, tableIndex, size, stripe, border, pageSizesArr, tableBtns, small, headBtns, tableCallback, rowClassName, isTableHeader, isLoadStatus } = defineProps({
    ...
    highlightCurrentRow: { type: Boolean, default: false, },             // 是否高亮显示当前选中行
...
defineExpose({ dataList, setTableDataHeight, selectionChange, checkSelect, toggleRowSelection, getTable, getAllCheck, setAllCheck, clearSelection, rowCheck, setTableLists, setTableTotal, setCurrentRow })
...
#tw_table {
    ...
    :deep(.el-table--striped .el-table__body .current-row td.el-table__cell) {
        background: #27CAFF !important;
    }
