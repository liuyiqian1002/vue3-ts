...
import { exportDataQualifiedStatistics, exportDataQualifiedStatisticsList} from '../../../../api/statisticalStatement/fiveFactorAnalysis/dataAcceptanceRate/index'
...
let mainData = reactive({
    tableUrl: '/statistics/fiveRateReport/queryDataQualifiedStatistics',
    ...
    columns: [
        ...
        { prop: 'inWholeVehicleCount', label: '不合格车辆数', formatter: (row)=> (<el-link onClick={()=>{
                title = '不合格车辆数明细'
                let rowObj = proxy.$_.cloneDeep(row), formData = proxy.$_.cloneDeep(mainData.formData)
                viewDataRef.viewData({...rowObj, ...formData, ...{type: 'DataQualified'}})
            }} type="primary">{row.inWholeVehicleCount}</el-link>)   },
        { prop: 'dataUnQualifiedRateCount', label: '不合格轨迹数', formatter: (row)=> (<el-link onClick={()=>{
                title = '不合格轨迹数明细'
                let rowObj = proxy.$_.cloneDeep(row), formData = proxy.$_.cloneDeep(mainData.formData)
                viewDataRef.viewData({...rowObj, ...formData, ...{type: ''}})
            }} type="primary">{row.dataUnQualifiedRateCount}</el-link>) },
        { prop: 'dataQualifiedRateCount', label: '总轨迹数' },
    ...
    headBtns: [
        {
            ...
            click: (row) => {
                ...
                proxy.MessageBox({ msg: txt, type: 'warning' }).then((close) => {
                    exportDataQualifiedStatistics(mainData.formData).then((res) => {
...
let mainDataBot = reactive({
    tableUrl: '/statistics/fiveRateReport/queryDataQualifiedStatisticsList',
    isLoadStatus: true,
    botHeight: 240,
    columns: [
        { prop: 'areaName', label: '所属区域' },
        ...
        { prop: 'dataQualifiedRateCount', label: '总轨迹数' },
        { prop: 'dataQualifiedRate', label: '数据合格率', sortable: true },
    ...
    headBtns: [
        {
            ...
            click: (row) => {
                let txt = `请确定导出全部数据`
                proxy.MessageBox({ msg: txt, type: 'warning' }).then((close) => {
                    exportDataQualifiedStatisticsList(mainData.formData).then((res) => {
                        window.open(res.fileUrl)
                        close()
                    }).catch(() => {
                        close()
