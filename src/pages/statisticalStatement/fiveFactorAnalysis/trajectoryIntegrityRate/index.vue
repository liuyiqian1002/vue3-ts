...
import { exportTrackWholeStatistics, exportTrackWholeStatisticsList} from '../../../../api/statisticalStatement/fiveFactorAnalysis/trajectoryIntegrityRate/index'
...
let mainData = reactive({
    tableUrl: '/statistics/fiveRateReport/queryTrackWholeStatistics',
    ...
    columns: [
        ...
        { prop: 'vehicleTotal', label: '上线车辆总数' },
        { prop: 'inWholeVehicleCount', label: '轨迹不完整车辆数', formatter: (row)=> (<el-link onClick={()=>{
                title = '轨迹不完整车辆数明细'
                let rowObj = proxy.$_.cloneDeep(row), formData = proxy.$_.cloneDeep(mainData.formData)
                viewDataRef.viewData({...rowObj, ...formData, ...{type: 'TrackWhole',url: '/statistics/fiveRateReport/queryVehicleList'}})
            }} type="primary">{row.inWholeVehicleCount}</el-link>)  },
        { prop: 'inWholeCount', label: '轨迹不完整轨迹数', formatter: (row)=> (<el-link onClick={()=>{
                title = '轨迹不完整轨迹数明细'
                let rowObj = proxy.$_.cloneDeep(row), formData = proxy.$_.cloneDeep(mainData.formData)
                viewDataRef.viewData({...rowObj, ...formData, ...{type: 'TrackWhole', url: '/statistics/fiveRateReport/queryLocationDriftDetail', status: 1}})
            }} type="primary">{row.inWholeCount}</el-link>) },
        { prop: 'trackWholeRate', label: '轨迹完整率' },
    ...
    headBtns: [
        {
            ...
            click: (row) => {
                ...
                proxy.MessageBox({ msg: txt, type: 'warning' }).then((close) => {
                    exportTrackWholeStatistics(mainData.formData).then((res) => {
...
let mainDataBot = reactive({
    tableUrl: '/statistics/fiveRateReport/queryTrackWholeStatisticsList',
    isLoadStatus: true,
    botHeight: 240,
    columns: [
        { prop: 'areaName', label: '所属区域' },
        ...
        { prop: 'trackWholeRate', label: '轨迹完整率', sortable: true },
    ...
    headBtns: [
        {
            ...
            click: (row) => {
                let txt = `请确定导出全部数据`
                proxy.MessageBox({ msg: txt, type: 'warning' }).then((close) => {
                    exportTrackWholeStatisticsList(mainData.formData).then((res) => {
                        window.open(res.fileUrl)
                        close()
                    }).catch(() => {
                        close()
