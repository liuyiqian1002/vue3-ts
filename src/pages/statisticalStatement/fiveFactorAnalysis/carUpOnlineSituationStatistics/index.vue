...
import { exportOnlineRate, exportOnlineRateList} from '../../../../api/statisticalStatement/fiveFactorAnalysis/carUpOnlineSituationStatistics/index'
...
let mainData = reactive({
    tableUrl: '/statistics/fiveRateReport/queryOnlineRate',
    ...
    columns: [
        ...
        { prop: 'vehicleTotal', label: '入网车辆数' },
        { prop: 'vehicleOnline', label: '在线数', formatter: (row)=> (<el-link onClick={()=>{
                title = '上线车辆明细'
                let rowObj = proxy.$_.cloneDeep(row), formData = proxy.$_.cloneDeep(mainData.formData)
                viewDataRef.viewData({...rowObj, ...formData, ...{type: 'Inline'}})
            }} type="primary">{row.vehicleOnline}</el-link>)  },
        { prop: 'vehicleNotOnline', label: '未上线数', formatter: (row)=> (<el-link onClick={()=>{
            title = '未上线车辆明细'
                let rowObj = proxy.$_.cloneDeep(row), formData = proxy.$_.cloneDeep(mainData.formData)
                viewDataRef.viewData({...rowObj, ...formData, ...{type: 'NotInline'}})
    ...
    headBtns: [
        {
            ...
            click: (row) => {
                ...
                proxy.MessageBox({ msg: txt, type: 'warning' }).then((close) => {
                    exportOnlineRate(mainData.formData).then((res) => {
...
let mainDataBot = reactive({
    tableUrl: '/statistics/fiveRateReport/queryOnlineRateList',
    isLoadStatus: true,
    botHeight: 240,
    columns: [
        { prop: 'areaName', label: '所属区域' },
    ...
    headBtns: [
        {
            ...
            click: (row) => {
                let txt = `请确定导出全部数据`
                proxy.MessageBox({ msg: txt, type: 'warning' }).then((close) => {
                    exportOnlineRateList(mainData.formData).then((res) => {
                        window.open(res.fileUrl)
                        close()
                    }).catch(() => {
                        close()
