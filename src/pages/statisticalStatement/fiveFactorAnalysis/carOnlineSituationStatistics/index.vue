...
import { exportOnlineStatistics, exportOnlineStatisticsList} from '../../../../api/statisticalStatement/fiveFactorAnalysis/carOnlineSituationStatistics/index'
...
let mainData = reactive({
    tableUrl: '/statistics/fiveRateReport/queryOnlineStatistics',
    ...
    columns: [
        ...
        { prop: 'vehicleNotOnline', label: '未在线数', formatter: (row)=> (<el-link onClick={()=>{
            title = '未在线车辆明细'
            viewDataRef.viewData(row)
            }} type="primary">{row.vehicleNotOnline}</el-link>) },
        { prop: 'onlineRate', label: '在线率' },
    ...
    headBtns: [
        {
            ...
            click: (row) => {
                ...
                proxy.MessageBox({ msg: txt, type: 'warning' }).then((close) => {
                    exportOnlineStatistics(mainData.formData).then((res) => {
...
let mainDataBot = reactive({
    tableUrl: '/statistics/fiveRateReport/queryOnlineStatisticsList',
    isLoadStatus: true,
    botHeight: 240,
    columns: [
        { prop: 'areaName', label: '所属区域' },
    ...
    headBtns: [
        {
            ...
            click: (row) => {
                let txt = `请确定导出全部数据`
                proxy.MessageBox({ msg: txt, type: 'warning' }).then((close) => {
                    exportOnlineStatisticsList(mainData.formData).then((res) => {
                        window.open(res.fileUrl)
                        close()
                    }).catch(() => {
                        close()
