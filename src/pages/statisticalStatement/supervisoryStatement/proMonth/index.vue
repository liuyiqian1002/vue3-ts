                                    ...
                                    <table border="1" class="daily-content" id="table">
                                        <tr>
                                            ...
                                            <th colspan="12" class="text-left subtitle">{{mainData.setData.filter(tag=> tag.parameterKey == 'transport')?.[0]?.parameterValue}}:<span
                                                id="report_districtName">{{mainData.formData.node?.orgName}}</span></th>
                                            <th colspan="5" class="text-left subtitle">填报时间:
                                                <span id="jkDate">{{mainData.formData.date}}</span></th>
                                        </tr>
                                        <tr>
...
let mainData = reactive({
    ...
    lists: [
        {label: '省名称', key: 'node', placeholder: '请选择组织结构', type: 'org', url: '/system/userDataAuthSearchTree/getTreesList', method: 'post', params: {hasDataPermission: true, needVehicle: false, searchType: 1, keyword: '', displayLevel: 1}, parKeys: [{ key: 'parentCode', val: 'permissionCode' }], props: {label: 'orgName', value: 'permissionCode', disabled: 'nocheck',}, permissionIds: [], disabledTypes: [1], isNodeType: true},
        ...
        { key: 'reportConfigId',label: '报表配置',type:'select', remote: true, url: '/statistics/report/reportConfigList', method: 'post', params: {reportType: 204}, props:{ label: 'reportConfigName', value:'reportConfigId'}, clearable: false, placeholder: '请选择报表配置', loadData: (lists)=> {
