...
let mainData = reactive({
    ...
    columns: [
        ...
        { prop: 'sendStatus', label: '结果', filter: 'sendStatus' },
        ...
        btns: [
            {
                ...
                click: (row) => {
                    proxy.MessageBox({ msg: '请确定下载当前数据!', type: 'warning' }).then((close) => {
                        ...
                        // window.open(row.emailAttachment)
                        close()
                    })
                },
            },
            ...
            click: (row) => {
                ...
                proxy.MessageBox({ msg: txt, type: 'warning' }).then((close) => {
                    ...
                    setBatchDownloadReqDataKey({reportTypeName: '监管报表自动发送', items: items}).then((res) => {
                        batchDownload({batchDownloadKey: res}).then((data) => {
                            proxy.download(data, `监管报表_${proxy.$dayjs().format('YYYY-MM-DD')}`, 'zip')
    ...
    lists: [
        ...
        {label: '报表类型', key: 'reportType', type: 'select', filter: 'superviseReportType', placeholder: '请选择报表类型',},
