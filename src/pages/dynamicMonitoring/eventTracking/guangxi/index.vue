<template>
    <div>
        ...
        <!--    详情    -->
        <CarDetailDialog ref="dialogRef" :title="title"></CarDetailDialog>
        <tw-dialog :loading="false" title="人工处理" :isBtns="false" width="45%" height="66%" v-model="manualHandleVisible" class="manual-dialog-wrapper">
...
let mainData = reactive({
    tableUrl: '/operation/alarmEventRelationEntity/pageTogether',
    isCheck: true,
    columns: [
        {
            prop: 'companyName',
    ...
    lists: [
        {label: '组织结构', key: 'typeIdList', showCheckbox: true, placeholder: '请输入组织结构', type: 'org', url: '/system/userDataAuthSearchTree/getTreesList', method: 'post', params: {hasDataPermission: true, needVehicle: false, searchType: 1, keyword: '',}, parKeys: [{ key: 'parentCode', val: 'permissionCode' }], props: {label: 'orgName', value: 'permissionCode', disabled: 'nocheck',}, permissionIds: [],},
        {label: '事件状态', key: 'eventStatusList', type: 'select', multiple: true, filter: 'eventStatus', placeholder: '请选择事件状态',},
        ...
        {label: '报警级别', key: 'alarmLevelMapList', type: 'allSelect', filter: 'eventAlarmTypes', placeholder: '请选择报警级别', returnObjectInArray: true, /* 返回数组里面的每一项是否是对象 */ returnObjectKeys: ['alarmLevel', 'alarmSubLevel'], /* [0]是大类值字段，[1]是子类值字段 */},
        {label: '报警类型', key: 'alarmTypeCodeList', type: 'allSelect', filter: 'alarmModeTypes', placeholder: '请选择报警类型', width: 760,},
        {
            ...
            change: (timeString: any) => {
                ...
                console.log(1,proxy.$dayjs(timeString).format('HH:mm:ss'));
                console.log(2,time);
        ...
        {label: '处理状态', key: 'hasManual', type: 'select', filter: 'eventHandleStatus', all: true,},
        ...
        {label: '企业名称', key: 'companyId', type: 'select', remote: true, url: '/business/company/associateCompany', method: 'post', params: { havePermission: true }, props: { label: 'companyName', value: 'companyId' }, placeholder: '请选择企业名称',},
        {label: '时间范围', key: ['startTime', 'endTime'], type: 'esDate', data: { type: 'datetime' }, col: 8,},
    ],
})
