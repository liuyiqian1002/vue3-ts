...
<!--                                    @save="save('company')"-->
                                ...
                                <div class="form_btns tw_c mt30">
                                    <el-button @click="resetForm">重置</el-button>
                                    <el-button type="primary" :loading="loading" @click="save('company')">保存</el-button>
                                </div>
                            </div>
                        </el-scrollbar>
                    </el-tab-pane>
                    <template v-if="editStatus">
                        <el-tab-pane label="证件信息" name="enterpriseCertificate">
                            <!--                    <service ref="systemRef" :formData="mainData.formData"></service>-->
                            <tw-table
...
let mainData = reactive({
    formData: {},
    headLlists: [
        {key: 'name', label: '企业名称', col: 12, placeholder: '请输入企业名称'},
        // {key: 'companyId', label: '企业名称', col: 12, type:'select', remote: true, url: '/business/company/associateCompany', method: 'post', params: {havePermission: true}, props:{ label: 'companyName', value:'companyId'}, placeholder: '请选择企业名称'},
    ...
    columns: [
                    ...
                    return (<el-form-item
                            ...
                            >
                                <el-select v-model={row.contactsRole} clearable>
                                    {
                                        store.getters.arrEnumerate.companyContactsRole.map(item => {
        ...
        {prop: "firstRoleTag", label: "是否第一联系人", width: 140,
            formatter: (row, column, cellValue, index) => {
                // if (index == mainData.formData.companyContactsListDtos.length - 1) {
                return (<el-form-item
                            label=" "
                            label-width={1}
                            prop={`companyContactsListDtos.${index}.firstRoleTag`}
                            rules={{required: true, message: '请选择是否第一联系人', trigger: 'change'}}
                        >
                            <el-select v-model={row.firstRoleTag} clearable>
                                <el-option label="是" value={true}/>
                                <el-option label="否" value={false}/>
                            </el-select>
                        </el-form-item>)
                // } else {
                //     return row.firstRoleTag?'是':'否'
                // }
            }},
        {prop: "contactsName", label: "姓名", width: 160, formatter: (row, column, cellValue, index) => (<el-form-item
                label=" "
                label-width={1}
        ...
        {prop: "cellphone", label: "手机", width: 160, formatter: (row, column, cellValue, index) => (<el-form-item
                label=" "
                label-width={1}
        ...
        {prop: "phone", label: "座机", width: 160, formatter: (row, column, cellValue, index) => customInput(row, index, 'phone')},
        {prop: "mail", label: "邮箱", width: 160, formatter: (row, column, cellValue, index) =>  customInput(row, index, 'mail')},
        {prop: "wechatId", label: "微信", width: 160, formatter: (row, column, cellValue, index) =>  customInput(row, index, 'wechatId')},
        {prop: "qq", label: "QQ", width: 160, formatter: (row, column, cellValue, index) => customInput(row, index, 'qq')},
    ],
    tableBtns: {
        prop: "btns", label: "操作", fixed: "right", width: 60, btns: [
            {
                icon: 'add', name: '新增', formatter:(row, column, index)=> index == mainData.formData.companyContactsListDtos.length-1, click: (row) => {
                    mainData.formData.companyContactsListDtos.push(proxy.$_.cloneDeep(editData.companyContactsListDtos[0]))
                }
            },
            {
                icon: 'shanchu', name: '删除', formatter:(row, column, index)=> index < mainData.formData.companyContactsListDtos.length-1 && index !== 0 , click: (row, column, index) => {
    ...
    lists: [
        {key: 'businessScope', label: '经营范围', placeholder: '请选择经营范围', type: 'select', col: 12, multiple: true, filter: 'companyBusinessScope'},
        {key: 'operState', label: '经营状态', placeholder: '请选择经营状态', type: 'select', col: 12, filter: 'companyOperatorState'},
        {key: 'companyType', label: '企业类别', placeholder: '请选择企业类别', type: 'select', col: 12, filter: 'companyType'},
        {key: 'postcode', label: '邮政编码', placeholder: '请输入邮政编码', col: 12},
    ...
    certificateHeadBtns: [
        {icon: 'add', name: '新增', click: (row, column, index)=> {
            if(mainData.formData.enterpriseCertificateListDtos.length<2){
                editTitle = '新增'
    ...
    certificateLists: [
        {prop: "credentialType", label: "证件类型", width: 120, filter: 'credentialType' },
        {prop: "credentialCode", label: "证件编码", width:140 },
        {prop: "issuingAuthority", label: "发证机构" , width:120},
        {prop: "startDate", label: "开始时间", width:120 },
        {prop: "endDate", label: "结束时间", width:120 },
        {prop: "remark", label: "备注", width:120 },
        {prop: "fileList", label: "附件", formatter: (row)=>{
            if(row.fileList.length>0){
                return (<div>
                        {row.fileList[0]?(<el-image className="tableUserImg radius" src={row.fileList[0]?.viewUrl}
                                                    preview-src-list={row.fileList[0]?.viewUrl}></el-image>):''}
                    {row.fileList[1]?(<el-image className="tableUserImg radius" src={row.fileList[1]?.viewUrl}
                                                preview-src-list={row.fileList[1]?.viewUrl}></el-image>):''}
                </div>)
    ...
    certificateTableBtns: {
        prop: "btns", label: "操作", fixed: "right", width: 80, btns: [
            {
                icon: 'bianji', name: '编辑', click: (row, column, index) => {
                    editTitle = `编辑`
                    basicsRef.setFormData(row,index)
                }
            },
            {
                icon: 'shanchu', name: '删除', formatter:(row, column, index)=> index !== 0 , click: (row) => {
                    // editTitle = `编辑字典类型`
    ...
    alterationLists: [
        {prop: "changedTime", label: "时间", width: 180 },
        {prop: "userAcount", label: "操作人" },
        {prop: "changedType", label: "变更类型" },
        {prop: "changedBefore", label: "变更前" },
        {prop: "changedAfter", label: "变更后" },
    ]
})
let formData = $ref({})
...
let basicsFormDataRef = $ref(null)
let map, marker
            ...
            map.on('click', function (e) {
                ...
                geocoder.getAddress(lnglatXY, function (status, result) {
                    if (status === 'complete' && result.info === 'OK') {
                        ...
                        mainData.formData.address = result.regeocode.formattedAddress
                    } else {
...
const setFormData = (obj) => {
    ...
    if (obj) {
        ...
        if (data.businessScope.indexOf(',') > 0) {
            ...
            data.businessScope = [data.businessScope].map(Number)
        }
...
const save = (str) => {
    fromDataRef.submitForm(false).then((form) => {
        console.log(str);
        if(str == 'company'){
            basicsFormDataRef.submitForm(false).then((form) => {
                apiUpdate(str)
            })
        }else {
            apiUpdate(str)
...
const apiUpdate = (str)=>{
    ...
    addOrUpdate(mainData.formData).then(() => {
        loading = false
