...
import {sendNotice, deleteById, getOneById} from "@/api/operation/businessmanagement/announcement";
...
let mainData = reactive({
    ...
    columns: [
        ...
        { prop: 'noticeType', label: '新闻类型', filter: 'noticeType'},
    ...
    tableBtns: { prop: "btns", label: "操作", fixed: "right", width: 160, btns:[
            ...
            {icon: 'fabu', name: '发布', formatter:(row)=> row.publishStatus, click: (row)=>{
    ...
    lists: [
        { label: '新闻标题', key: 'title', placeholder: '请输入新闻标题' },
        { label: '新闻类型', key: 'noticeType', type:'select',placeholder: '请选择新闻类型',filter: 'noticeType' },
        { label: '发布状态', key: 'publishStatus', type:'select', filter: 'publishType'},
