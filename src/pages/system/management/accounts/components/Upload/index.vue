<template>
    <div id="tw_user_upload relative" class="tw_cy tw_user_upload">
        <div class="upload_img tw_c relative">
            <div class="upload_close radius tw_c positionTr" v-show="modelValue" style="z-index: 1">
                <el-icon :size="16" class="pointer" @click="close"><Close /></el-icon>
            </div>
            <el-image style="width: 100px; height: 100px" class="radius" :src="modelValue" :zoom-rate="1.2" :preview-src-list="[modelValue]">
                <template #error>
...
<!--        :action="'api/file/uploadImage'"-->
        ...
        >
            <template #trigger>
                <el-button icon="upload-filled">上传文件</el-button>
            </template>
...
const { uploadType, id, tipShow, sex, multiple, limit, modelValue } = defineProps({// 文件类型, 例如['png', 'jpg', 'jpeg']
...
let emits = defineEmits(['update:modelValue', 'update:id', 'handleSuccess', 'clearUpload'])
//上传成功回调
